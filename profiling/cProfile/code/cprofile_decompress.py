...
result=StringIO()
stats = Stats("cprofile_decompress.prof",stream=result)
stats.strip_dirs()
...
f=open('cprofile_decompress.csv','w')
f.write(result)
f.close()
...
df = pd.read_csv('cprofile_decompress.csv',skipinitialspace=True)
...
g.fig.suptitle('cprofile_decompress', weight='bold', fontsize=16)
plt.show()
