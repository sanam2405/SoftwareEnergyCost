...
result=StringIO()
stats = Stats("cprofile_compress.prof",stream=result)
stats.strip_dirs()
...
f=open('cprofile_compress.csv','w')
f.write(result)
f.close()
...
df = pd.read_csv('cprofile_compress.csv',skipinitialspace=True)
...
g.fig.suptitle('cProfile_compress', weight='bold', fontsize=16)
plt.show()
